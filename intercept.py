...
#
# INSTALL on the Pi (correct layout):
#   Helper modules go in ~/cowrie/src/cowrie/llm/ (NOT commands/).
#   Only this file goes in ~/cowrie/src/cowrie/commands/ as scalpel_intercept.py.
#
#     scp tier1_static.py              pi:~/cowrie/src/cowrie/llm/
#     scp tier2_classifier.py          pi:~/cowrie/src/cowrie/llm/
#     scp tier3_ollama_tier4_cloud.py  pi:~/cowrie/src/cowrie/llm/
#     scp system_facts.py              pi:~/cowrie/src/cowrie/llm/
#     scp cowrie_intercept.py          pi:~/cowrie/src/cowrie/commands/scalpel_intercept.py
#     touch ~/cowrie/src/cowrie/llm/__init__.py
...
# Tier modules live in cowrie.llm when running under Cowrie. Fall back to
# sys.path-based imports for standalone smoke tests.
try:
...
try:
    ...
    # Catch-all registration — only honored if you patch protocol.py/honeypot.py
    # to route through SCALPEL; stock Cowrie doesn't treat "*" as a wildcard.
